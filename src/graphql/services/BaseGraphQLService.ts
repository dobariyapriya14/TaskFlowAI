...
export abstract class BaseGraphQLService {
  ...
  /**
   * Set a custom ApolloClient instance for this service
   */
    ...
    try {
      ...
      if (response.errors && response.errors.length > 0) {
        response.errors.forEach(err => {
          const msg = `[BaseGraphQLService.query] Error: ${err.message}`;
          console.warn(msg);
      ...
      const msg = `[BaseGraphQLService.query] Exception: ${
        error.message || error
      }`;
      console.error(msg);
      this.logToCrashlytics(msg);
  ...
  >(
    ...
    try {
      ...
      if (response.errors && response.errors.length > 0) {
        response.errors.forEach(err => {
          const msg = `[BaseGraphQLService.mutate] Error: ${err.message}`;
          console.warn(msg);
          this.logToCrashlytics(msg);
        });
      }
      ...
      const msg = `[BaseGraphQLService.mutate] Exception: ${
        error.message || error
      }`;
      console.error(msg);
      this.logToCrashlytics(msg);
  ...
  public writeQuery<TData = any, TVariables = OperationVariables>(
    ...
    try {
      ...
      console.warn('[BaseGraphQLService.writeQuery] Error:', error);
    }
  }
  ...
  private logToCrashlytics(message: string): void {
    try {
